...
public class BlogService {
    ...
    public Blog selectById(Integer id) {
        ...
        beAttention+=attentionService.selectByFidCount(user.getId());
        user.setUserLikesCount(beLikes);
        user.setUserCollectCount(beCollect);
        user.setUserAttentionCount(beAttention);
        //判断作者是否被当前用户关注
        Account currentUser = TokenUtils.getCurrentUser();
        user.setUserAttention(attentionService.selectUserAttention(user.getId())!=null);
...
//        blogMapper.updateById(blog);
    ...
    public PageInfo<Blog> selectAttentionBlogs(Integer pageNum, Integer pageSize) {
        Integer userId=null;
        Account currentUser = TokenUtils.getCurrentUser();
        if(RoleEnum.USER.name().equals(currentUser.getRole())){
            userId=currentUser.getId();
        ...
        PageHelper.startPage(pageNum, pageSize);
        List<Blog> list = blogMapper.selectAttentionBlogs(userId);
        for (Blog item :list) {
            int i = likesService.selectByFidAndModule(item.getId(), LikesModuleEnum.BlOG.getValue());
            item.setUser(userService.selectById(item.getUserId()));
            item.setLikesCount(i);
